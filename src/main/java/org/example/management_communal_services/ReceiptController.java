...
// Контроллер для окна просмотра и формирования квитанции
// Отображает начисления в TableView и формирует DOCX-файл
public class ReceiptController {

    // Лейблы для отображения данных собственника
    @FXML private Label lblAccountNumber;
    @FXML private Label lblFullName;
    @FXML private Label lblAddress;
    @FXML private Label lblPeriod;
    @FXML private Label lblPeriodDates;  // Новый лейбл для дат периода

    // Лейблы для итоговых расчётов
    @FXML private Label lblDebt;
    @FXML private Label lblCurrentCharges;
    @FXML private Label lblTotal;
    @FXML private Label lblInfoText;  // Лейбл для справочной информации

    // TableView для отображения начислений
    ...
    // Контейнер для жилищных услуг
    @FXML private VBox housingContainer;

    // Данные текущего владельца и периода
    private int currentOwnerId;
    private int currentMonth;
    private int currentYear;
    private double totalAmount = 0.0;

    // Внутренний класс для строки таблицы начислений
    // Хранит все данные для отображения в TableView и экспорта в DOCX
    public static class ChargeRow {
        ...
        // Геттеры для привязки колонок TableView
    ...
    // Установка данных владельца и периода для квитанции
    // Вызывается из MetersController после загрузки FXML
    ...
    // Загрузка всех данных для квитанции
    // Последовательно загружает информацию о собственнике, начисления и считает итог
    private void loadReceiptData() {
        loadOwnerInfo();
        loadChargesTable();
        loadHousingServices();
        calculateTotal();
        loadInfoText();  // Загружаем справочную информацию
    }

    // Загрузка информации о собственнике из таблицы Owners
    // Заполняет лейблы с лицевым счётом, ФИО, адресом и периодом
    private void loadOwnerInfo() {
        ...
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            if (rs.next()) {
                ...
                lblAddress.setText("Адрес: " + rs.getString("street") +
                        ", д. " + rs.getString("building") +
                        ", кв. " + rs.getString("apartment_number"));

                // Форматируем период с датами: "25.04.2026 - 30.05.2026"
    ...
    // Загрузка начислений и заполнение TableView
    // Объединяет данные из Charges, Tariffs и MeterReadings
    private void loadChargesTable() {
        // Получаем ТЕКУЩИЕ показания (за месяц квитанции)
        String currentReadingsSql = "SELECT electricity, hot_water, cold_water " +
                "FROM MeterReadings WHERE owner_id = ? AND month = ? AND year = ?";

        // Получаем ПРЕДЫДУЩИЕ показания (последние до текущего месяца)
        String lastReadingsSql = "SELECT electricity, hot_water, cold_water " +
                "FROM MeterReadings WHERE owner_id = ? AND (year < ? OR (year = ? AND month < ?)) " +
                "ORDER BY year DESC, month DESC LIMIT 1";

        // SQL для получения начислений с тарифами за текущий период
        String chargesSql = "SELECT c.volume, c.amount, c.tariff_price, c.month, c.year, " +
                "t.service_name, t.unit, t.normative, t.category " +
                "FROM Charges c " +
                "LEFT JOIN Tariffs t ON c.tariff_id = t.id " +
                "WHERE c.owner_id = ? AND c.month = ? AND c.year = ? " +
                "ORDER BY t.category, t.service_name";

        try (Connection conn = DatabaseConnector.getConnection()) {

            // 1. Получаем текущие показания за месяц квитанции
            PreparedStatement currStmt = conn.prepareStatement(currentReadingsSql);
            currStmt.setInt(1, currentOwnerId);
            currStmt.setInt(2, currentMonth);
            ...
            // 2. Получаем предыдущие показания для расчёта РАСХОДА
            ...
            // 3. Получаем начисления за текущий период
            ...
            while (chargesRs.next()) {
                ...
                // Показания и объём зависят от типа услуги
                String readings = "-";
                String volumeText = "-";

                // Проверяем, является ли услуга счётчиковой
                boolean isMeteredService = false;
                double consumption = 0; // Расход
                ...
                if (serviceName.toLowerCase().contains("электро") ||
                    ...
                    if (hasCurrentReadings) {
                        ...
                        if (hasLastReadings) {
                            ...
                            consumption = currElectricity; // Если нет предыдущих, считаем от 0
                        }
                    }
                }
                // ГВС (теплоноситель) - это вода
                ...
                if (isMeteredService && hasCurrentReadings) {
                    ...
                    // Для услуг без счётчиков берём из Charges
                ...
                // Создаём строку таблицы и добавляем в TableView
                ChargeRow row = new ChargeRow(serviceName, unitText, readings, volumeText,
                        tariffText, normativeText, amountText, month, year);
                chargesTable.getItems().add(row);

                // Считаем общую сумму начислений
                totalAmount += amount;
            }
    ...
    // Загрузка жилищных услуг, заказанных Собственником
    // Показываем только услуги со статусом "В работе"
    private void loadHousingServices() {
        String sql = "SELECT s.name, s.price " +
                "FROM Applications a " +
    ...
    private void loadInfoText() {
        String infoText = "Показания приборов учёта необходимо передавать ежемесячно. " +
                "Показания приборов учёта нужно вводить раз в месяц с промежутком не менее 15 дней " +
                "между предыдущими и текущими показаниями приборов учёта.\n";
    ...
    // Расчёт итоговой суммы и задолженности
    // Учитывает неоплаченные начисления и фактические платежи
    private void calculateTotal() {
        // Считаем задолженность: сумма неоплаченных начислений за прошлые периоды
        ...
        try (Connection conn = DatabaseConnector.getConnection();
            ...
            // Считаем фактические платежи из таблицы Payments
            ...
            try (PreparedStatement payStmt = conn.prepareStatement(paymentsSql)) {
                ...
                // Рассчитываем баланс: переплата или задолженность
                ...
                lblTotal.setText(String.format("ИТОГО К ОПЛАТЕ: %.2f руб.", totalAmount + Math.max(0, -balance)));
    ...
    // Обработчик кнопки "Сформировать квитанцию"
    // Открывает диалог сохранения и создаёт DOCX-файл
    @FXML
    private void generateReceipt() {
        ...
        if (file == null) return;  // Пользователь отменил сохранение
    ...
    // Создание DOCX-файла с помощью Apache POI
    // Формирует таблицу с начислениями и итоговой суммой
    private void createDOCX(String filePath) throws IOException {
        ...
        // Заголовок раздела начислений
        addSectionTitle(document, "Начисления по услугам");

        // Создаём таблицу: 8 колонок, строки = данные + заголовок
        ...
        // Заполняем строки таблицы данными из TableView
        ...
        infoRun.setText("Показания приборов учёта необходимо передавать ежемесячно. ");
        infoRun.setText("Показания приборов учёта нужно вводить раз в месяц с промежутком не менее 15 дней ");
        infoRun.setText("между предыдущими и текущими показаниями приборов учёта.\n");
        ...
        for (var node : housingContainer.getChildren()) {
            if (node instanceof Label) {
                addParagraph(document, ((Label) node).getText());
            }
        }
        addEmptyLine(document);
    ...
    // Настраиваем привязку колонок TableView к полям ChargeRow
    @FXML
    public void initialize() {
        serviceNameColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getServiceName()));

        unitColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getUnit()));

        readingsColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getReadings()));

        volumeColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getVolume()));

        tariffColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getTariff()));

        normativeColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getNormative()));

        calcMonthColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(
                        getMonthName(cellData.getValue().getMonth()) + " " + cellData.getValue().getYear()));

        amountColumn.setCellValueFactory(cellData ->
                new javafx.beans.property.SimpleStringProperty(cellData.getValue().getAmount()));
    }

    // Вспомогательный метод: добавляет абзац текста в документ
    ...
    // Вспомогательный метод: добавляет заголовок раздела
    ...
    // Вспомогательный метод: добавляет пустую строку
